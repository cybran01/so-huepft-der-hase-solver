...
fn main() {
    ...
    let mut parent = graph.edgemap[&Unique(winning_board.clone().unwrap())].clone();
    println!("{parent}");
    while parent != graph.root {
        let next_parent = graph.edgemap[&Unique(parent.clone())].clone();
        parent = next_parent;
        println!("{parent}");
    }
}
